...
if [ "$MODE" = "infer" ]; then
    ...
    # training/recon-worker split (and none of the multi-trainer pool-dir race). Alignment
    # uses all visible GPUs, so we expose them via CUDA_VISIBLE_DEVICES.
    ...
    echo "Running miss-alignment (training)…"
    miss-alignment \
        --config-file "$CONFIG" \
