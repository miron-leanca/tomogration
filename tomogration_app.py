...
Scope: raw data prep -> particle export (ts_export_particles). Averaging
(RELION 5 / M) is OUT of scope — it lives in those tools' own pipeliners.
...
class ProjectState:
    ...
    def status_exported(self):
        n = self.count_rglob("relion", "*.star")
        return (n, f"{n} .star in relion/" if n else None)
...
STAGES = [
    ...
    {
        # ---------- SCOPE ENDPOINT ----------
        "group": "9. Export (scope end)", "id": "ts_export_particles",
        ...
        "params": [
            ...
            {"name": "output_star", "kind": "text", "flag": "--output_star",
             "default": "relion/matching.star", "help": "Output star path."},
            {"name": "output_angpix", "kind": "slider_int", "flag": "--output_angpix",
             "default": 4, "min": 1, "max": 10, "step": 1,
             "help": "Export pixel size (Å). Choose so Nyquist sits just below "
                     "feature resolution."},
            {"name": "box", "kind": "slider_int", "flag": "--box",
             "default": 64, "min": 32, "max": 192, "step": 16, "help": "Box size (px)."},
            {"name": "diameter", "kind": "text", "flag": "--diameter",
             "default": "", "help": "Particle diameter (Å)."},
            {"name": "twod", "kind": "check", "flag": "--2d",
             "default": True, "help": "2D image series (RELION 5). Off = 3D subtomos."},
            ...
            {"name": "relative_output_paths", "kind": "check",
             ...
             "help": "Write relative paths into the star (portable projects)."},
        ],
        "docs": {
            "what": "Extracts CTF-corrected particles -> RELION optimisation_set.star.",
            "range": "box 64-128; output_angpix 3-5 for most targets.",
            "effect": "--2d gives a RELION-5-ready 2D image series; omit for 3D volumes.",
            "pitfall": "This is the handoff point. Averaging (RELION 5 --tomo / M) "
                       "happens in those tools' own pipeliners, OUTSIDE tomogration.",
        },
...
STAGE_OUTPUTS = {
    ...
    "threshold_picks": "warp_tiltseries/matching", "ts_export_particles": "relion",
}
...
STAGE_IO = {
    ...
    "ts_export_particles":  (["warp_tiltseries", "warp_tiltseries/matching"], ["relion"]),
}
