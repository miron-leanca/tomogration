...
def main():
    ...
    # The Class3D PAD block that used to live here went with the stage
    # (relion4_class3d, removed 2026-08-21 — never run through tomogration).
    # ml_relion4_handoff_warp_auto.sh is now referenced by no stage at all;
    # left on disk deliberately rather than deleted, since it is the user's
    # script to keep or bin.
    check("no stage references the orphaned RELION handoff script",
          not any("relion4_handoff" in str(q.get("default", ""))
                  for x in st.STAGES for q in x.get("params", [])))
